const MAX_YOUTUBE_UPLOAD_TITLE_LENGTH = 100;
...
const titleAlreadyIncludesArtist = (title = '', artistName = '') => {
  ...
  const exactArtist = cleanTitle.toLowerCase() === cleanArtist.toLowerCase();
  if (exactArtist) {
    return true;
  }
...
const getYoutubeUploadTitleInfo = ({
  title = '',
  artistName = '',
  featuringArtists = [],
  fallbackTitle = 'Untitled Video',
...
const buildYoutubeUploadTitle = (options = {}) =>
  getYoutubeUploadTitleInfo(options).uploadTitle;

module.exports = {
  MAX_YOUTUBE_UPLOAD_TITLE_LENGTH,
